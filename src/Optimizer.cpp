...
void Optimizer::export_obj(Mesh &mesh, int frame) {
    ...
    std::ofstream surface_stream(path + "/surface_" + std::to_string(frame) + ".obj");
    surface_stream << "# Generated by iARAP"
                   << "\n";
    surface_stream << std::fixed << std::setprecision(6) << "surface\n";
    auto vertices = mesh.pos;
    auto triangles = mesh.surface;

    for (auto vec: vertices) {
        surface_stream << "v " << vec(0) << " " << vec(1) << " "
                       << vec(2) << "\n";
    }
    surface_stream << "s 1\n";
    for (auto tri: triangles) {
        surface_stream << "f " << tri(0) + 1 << " " << tri(1) + 1
                       << " " << tri(2) + 1 << "\n";
    }
